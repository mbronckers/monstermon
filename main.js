...
var map = {cols: 32, rows: 32, tileSize: 16,
    tiles: [
    ...
    for (var r = 0; r < map.rows; r++) {
        for (var c = 0; c < map.cols; c++) {
            ...
            if (tile != -1) { // a value of -1 in the map array means empty tile
                ...
                if (tile == grassValue) {            // a case for each type of tile in array --> grass
                    ctx.drawImage(
                ...
                if (tile == blackValue) {            // a case for each type of tile in array --> black square
                    ctx.drawImage(
                        texture,            // image file source
                        (blackY * map.tileSize), // the x coordinate of the image file to clip --> texture file tile's x position
                        (blackX * map.tileSize), // the y coordinate of the image file to clip --> texture file tile's y position
                        map.tileSize, 		// the width of the clipped image --> texture file tile's width
                        map.tileSize,		// the height of the clipped image --> texture file tile's height
                        c * (map.tileSize * 2), 	// x position of tile on canvas
                        r * (map.tileSize * 2),	// y position of tile on canvas
